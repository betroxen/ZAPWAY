...
const MULTIPLIERS = {
    LOW: { 8: [5.6,2.1,1.1,1,.5,1,1.1,2.1,5.6], 10: [8.9,3,1.4,1.1,1,.5,1,1.1,1.4,3,8.9], 12: [10,3,1.6,1.4,1.1,1,.5,1,1.1,1.4,1.6,3,10], 14: [7.1,4,1.9,1.4,1.3,1.1,1,.5,1,1.1,1.3,1.4,1.9,4,7.1], 16: [16,9,2,1.4,1.4,1.2,1.1,1,.5,1,1.1,1.2,1.4,1.4,2,9,16] },
    MEDIUM: { 8: [13,3,1.3,.7,.4,.7,1.3,3,13], 10: [22,5,2,1.4,.6,.4,.6,1.4,2,5,22], 12: [33,11,4,2,1.1,.6,.3,.6,1.1,2,4,11,33], 14: [58,15,7,4,1.9,1,.5,.2,.5,1,1.9,4,7,15,58], 16: [110,41,10,5,3,1.5,1,.5,.3,.5,1,1.5,3,5,10,41,110] },
    HIGH: { 8: [29,4,1.5,.3,.2,.3,1.5,4,29], 10: [76,10,3,.9,.3,.2,.3,.9,3,10,76], 12: [170,24,8.1,2,.7,.2,.2,.2,.7,2,8.1,24,170], 14: [420,56,18,5,1.9,.3,.2,.2,.2,.3,1.9,5,18,56,420], 16: [1e3,130,26,9,4,2,.2,.2,.2,.2,.2,2,4,9,26,130,1e3] }
};
...
export const PlinkoGamePage = () => {
    ...
    const lastBucketHitRef = useRef(null);
    ...
    const dropBall = useCallback(() => {
        ...
        particlesRef.current.push({ id: Math.random().toString(36).substr(2, 9), x: width / 2 + (Math.random() - 0.5) * 10, y: 30, vx: 0, vy: 0, radius: 5, path, currentRow: 0, finished: false, history: [] });
    ...
    useEffect(() => {
        ...
        const render = (time) => {
            ...
            multipliers.forEach((mult, i) => {
                ...
                ctx.beginPath();
                if (ctx.roundRect) { ctx.roundRect(x - w / 2, bucketY, w, bucketHeight, 4); } else { ctx.rect(x - w / 2, bucketY, w, bucketHeight); }
                ctx.fill();
            ...
            particlesRef.current.forEach((p) => {
                ...
                p.vy += 0.8 * deltaTime; p.vy *= 0.98; p.vx *= 0.97; p.x += p.vx * deltaTime; p.y += p.vy * deltaTime;
                for (let j = 0; j < pegsRef.current.length; j++) {
                     const peg = pegsRef.current[j];
                     if (Math.abs(peg.y - p.y) < 20 && Math.abs(peg.x - p.x) < 20) {
                         const dx = p.x - peg.x; const dy = p.y - peg.y; const dist = Math.sqrt(dx*dx + dy*dy);
                         if (dist < p.radius + peg.r) {
                             peg.lastHit = time; const direction = p.path[p.currentRow]; p.vy = -Math.abs(p.vy) * 0.5 - 2;
                             const guideForce = (Math.random() * 1.5 + 2); p.vx += direction === 0 ? -guideForce : guideForce;
                             p.y = peg.y - (p.radius + peg.r + 1); p.currentRow++; playSound('plinko_hit'); break;
                         }
                     }
                }
                if (p.y > bucketY - p.radius) {
                    p.finished = true; p.y = bucketY - p.radius;
                    const pathSum = p.path.reduce((a, b) => a + b, 0); const payoutMult = multipliers[pathSum] || 0;
                ...
                for (let h = 0; h < p.history.length - 1; h++) {
                    const point = p.history[h]; const nextPoint = p.history[h+1];
                    ctx.globalAlpha = h / p.history.length; ctx.moveTo(point.x, point.y); ctx.lineTo(nextPoint.x, nextPoint.y);
                }
    ...
    useEffect(() => {
        let interval;
        if (autoBetActive) { interval = setInterval(dropBall, 250); }
        return () => clearInterval(interval);
    }, [autoBetActive, dropBall]);

    useEffect(() => {
        const handleKeyPress = (e) => { if (e.code === 'Space' && !e.repeat && !pfModalOpen) { e.preventDefault(); dropBall(); } };
        window.addEventListener('keydown', handleKeyPress);
        return () => window.removeEventListener('keydown', handleKeyPress);
    }, [dropBall, pfModalOpen]);

    return (
        <div className="container mx-auto max-w-7xl p-4 py-6 md:p-8 min-h-[calc(100vh-8rem)] flex flex-col">
                        ...
                        <div>
                            ...
                            <div className="relative flex items-center">
                             ...
                             <div className="grid grid-cols-4 gap-2 mt-2">
                                {['MIN', '1/2', '2x', 'MAX'].map(opt => <button key={opt} onClick={() => { if (opt === 'MIN') setBetAmount(1); if (opt === '1/2') setBetAmount(Math.max(1, betAmount / 2)); if (opt === '2x') setBetAmount(betAmount * 2); if (opt === 'MAX') setBetAmount(simBalance); }} className="bg-[#14131c] border border-[#333] rounded text-[10px] font-heading py-2 text-[#8d8c9e] hover:text-white hover:border-[#8d8c9e] transition-all active:scale-95">{opt}</button>)}
                            </div>
                        </div>
                        <div>
                         ...
                         <div>
                            ...
                            <select value={rows} onChange={(e) => setRows(Number(e.target.value))} className="w-full h-12 bg-[#0A0A0A] border border-[#333] rounded px-4 text-sm text-white font-mono focus:border-[#00FFC0] outline-none appearance-none">{ROW_OPTIONS.map(r => <option key={r} value={r}>{r} ROWS</option>)}</select>
                        </div>
                    </div>
            ...
            <ProvablyFairModal isOpen={pfModalOpen} onClose={() => setPfModalOpen(false)} gameName="PLINKO" serverSeedHash={serverSeedHash} clientSeed={clientSeed} nonce={nonce} onRotateSeeds={(newSeed) => { setClientSeed(newSeed); setNonce(0); showToast("SEEDS ROTATED. LEDGER RESET.", "success"); }} />
        </div>
    );
};