...
export const MinesGamePage = () => {
    ...
    const { playSound } = useSound(); // Hook into audio engine
    ...
    const handleTileClick = (index: number) => {
        ...
        if (tile.isMine) {
            ...
            setGameState('busted');
            revealAll(newGrid);
            playSound('mine_boom', 0.8); // Loud explosion sound
        } else {
            tile.state = 'safe';
            setRevealedCount(prev => prev + 1);
            playSound('mine_safe', 0.4); // Crisp tick sound
    ...
    const cashout = (currentGrid = grid) => {
        ...
        playSound('cashout', 0.7); // Satisfying ka-ching/credits sound
    };
                    ...
                    <div className="p-5 pt-0 mt-auto">
                        {gameState === 'playing' ? (
                             <Button 
                                onClick={() => cashout()} 
                                size="lg"
                                disableSound={true} // Use custom sound in cashout() instead of generic click
                            ...
                            <Button onClick={startGame} size="lg" disableSound={true} className="w-full py-6 font-orbitron font-bold uppercase tracking-widest text-lg shadow-[0_0_30px_rgba(0,255,192,0.15)] bg-[#00FFC0] text-black hover:scale-[1.02] active:scale-95 transition-all">
                                DEPLOY PROBE
                            </Button>
                        )}
                    </div>
                                        ...
                                        return (
                                            <div key={tile.id} className="relative w-full h-full perspective-500">
                                                <button
                                                    ...
                                                    onMouseEnter={() => { if(isInteractable) playSound('click_secondary', 0.05); setIsHoveringTile(index); }}
                                ...
                                {(gameState === 'busted' || gameState === 'cashed_out') && (
                                    <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/60 backdrop-blur-[4px] animate-fadeIn rounded-[28px]">
                                        <div className={`p-8 md:p-10 rounded-3xl border-2 text-center transform scale-105 shadow-[0_0_60px_rgba(0,0,0,0.8)] flex flex-col items-center ${gameState === 'busted' ? 'bg-[#0c0c0e] border-red-900/50' : 'bg-[#0c0c0e] border-[#00FFC0]/30'}`}>
                                            ...
                                            <Button onClick={startGame} disableSound={true} size="lg" className={`px-12 py-6 font-orbitron font-black uppercase tracking-widest text-base ${gameState === 'busted' ? 'bg-[#333] hover:bg-white hover:text-black text-white shadow-none' : 'shadow-[0_0_30px_rgba(0,255,192,0.4)]'}`}>
                                                REDEPLOY
                                            </Button>
                                        </div>
                                    </div>
