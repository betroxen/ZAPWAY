...
export const MinesGamePage = () => {
    ...
    const [isHoveringTile, setIsHoveringTile] = useState(null);
    const [pfModalOpen, setPfModalOpen] = useState(false);
    const [serverSeedHash, setServerSeedHash] = useState('e4b5c6d7e8...');
    ...
    const startGame = () => {
        if (betAmount <= 0) { showToast("INVALID BET AMOUNT.", "error"); return; }
        if (betAmount > simBalance) { showToast("INSUFFICIENT FUNDS.", "error"); return; }
        
        playSound('game_start', 0.3);
        setSimBalance(prev => prev - betAmount);
        setGameState('playing');
        setRevealedCount(0);
        setNonce(prev => prev + 1);

        const newGrid = Array.from({ length: 25 }, (_, i) => ({ id: i, isMine: false, state: 'hidden' }));
        let minesPlaced = 0;
    ...
    const handleTileClick = (index) => {
        ...
        if (tile.isMine) {
            playSound('mine_boom', 0.4);
            tile.state = 'exploded';
            setGameState('busted');
            revealAll(newGrid);
        } else {
            playSound('mine_safe', 0.5);
            tile.state = 'safe';
    ...
    const revealAll = (currentGrid, exploded = true) => {
    ...
    const handleRotateSeeds = (newClientSeed) => {
                            ...
                            <span>START AMT:</span>
                             <input 
                                ...
                                onChange={(e) => {
                                    ...
                                    setStartBalance(val);
                                    if (gameState === 'idle') setSimBalance(val);
                                }}
                                            ...
                                            <div key={tile.id} className="relative w-full h-full">
                                                ...
                                                >
                                                    ...
                                                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden rounded-xl">
                                                        {tile.state === 'safe' && <img src={ASSETS.GEM} alt="Safe" className="w-[75%] h-[75%] object-contain drop-shadow-[0_0_15px_rgba(0,255,192,0.5)] animate-[bounce_0.4s_ease-out]" />}
                                                        {tile.state === 'exploded' && <img src={ASSETS.MINE} alt="EXPLODED" className="w-[80%] h-[80%] object-contain drop-shadow-[0_0_35px_rgba(239,68,68,0.9)] animate-pulse-fast" />}
                                                        {tile.state === 'mine' && <img src={ASSETS.MINE} alt="Mine" className="w-[65%] h-[65%] object-contain opacity-50 grayscale" />}
                                                    </div>
                                                </button>
                                            </div>
                                        );
                                    ...
                                    <div className="absolute inset-0 z-30 flex items-center justify-center bg-black/60 backdrop-blur-[4px] animate-fadeIn rounded-[28px]">
                                        <div className={`p-8 md:p-10 rounded-3xl border-2 text-center transform scale-105 shadow-[0_0_60px_rgba(0,0,0,0.8)] flex flex-col items-center ${gameState === 'busted' ? 'bg-[#0c0c0e] border-red-900/50' : 'bg-[#0c0c0e] border-[#00FFC0]/30'}`}>
                                            <div className={`mb-4 p-4 rounded-full border-2 ${gameState === 'busted' ? 'bg-red-500/10 border-red-500/20' : 'bg-[#00FFC0]/10 border-[#00FFC0]/20'}`}>
                                                {gameState === 'busted' ? <Icons.Bomb className="h-12 w-12 text-red-500" /> : <Icons.Shield className="h-12 w-12 text-[#00FFC0]" />}
                                            </div>
                                            <h2 className={`font-orbitron text-4xl md:text-5xl uppercase font-black mb-2 text-white`}>
