...
export const ReviewModal: React.FC<ReviewModalProps> = ({ isOpen, onClose, initialCasinoId }) => {
  ...
  return (
    <div className="fixed inset-0 z-[70] overflow-y-auto" role="dialog" aria-modal="true">
      {/* Darkened Backdrop (Fixed) */}
      <div 
          className="fixed inset-0 bg-[#000000]/90 backdrop-blur-md transition-opacity" 
          onClick={onClose} 
          aria-hidden="true" 
      />

      {/* Centering Container (Scrollable) */}
      <div className="flex min-h-full items-center justify-center p-4 text-center">
          
          {/* MAIN TERMINAL CONTAINER */}
          <div className="relative w-full max-w-3xl rounded-xl bg-[#14131c] border border-[#3a3846] shadow-2xl animate-materialize flex flex-col text-left overflow-hidden my-8">
            
            {/* PROTOCOL HEADER */}
            <div className="p-6 border-b border-[#3a3846] bg-[#0c0c0e]">
            ...
            {/* Added max-h to ensure internal scrolling on very small vertical screens if needed, though min-h-full on parent usually handles it. */}
            <div className="p-6 md:p-8 overflow-y-auto custom-scrollbar bg-[#14131c] max-h-[60vh]">
                                ...
                                <label className="block text-xs font-mono text-[#8d8c9e] uppercase mb-2">Target Operator (The Subject)</label>
                                 {selectedCasino ? (
                                    ...
                                    <div className="relative">
                                        ...
                                        />
                                        {searchTerm && (
                                            <div className="absolute top-full left-0 w-full bg-[#14131c] border border-[#3a3846] rounded-md mt-1 max-h-40 overflow-y-auto z-20 shadow-xl">
                                                {filteredCasinos.map(c => (
                                                    <button key={c.id} onClick={() => { handleInputChange('targetOperator', c.id); setSearchTerm(''); }} className="w-full text-left p-3 hover:bg-[#24232d] flex items-center gap-3 transition-colors border-b border-[#333] last:border-0">
                                                        <img src={c.logo} className="w-6 h-6 rounded" alt="" /> <span className="text-white font-bold">{c.name}</span>
                                                    </button>
                                                ))}
                                            </div>
                                        )}
                ...
                {currentStep === 4 && (
                    <div className="animate-fadeIn space-y-8">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <Icons.Link className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8d8c9e] h-5 w-5" />
                                <Input 
                                    placeholder="https://..." 
                                    className="pl-10 font-mono"
                                    type="url"
